var Index = function () {
    ...
    return {
        ...
        init : function(){
            Index.showSensorMarkerItv =  setInterval(Index.showSensorMarker,100);

        },

        showSensorMarker:function() {

            if(Gis.getGlobalControl()==null){
                return;
            }
            var layer = Gis.getGlobalControl().Globe.Layers.GetLayerByCaption("传感设备图层");
            var liquid_layer = Gis.getGlobalControl().Globe.Layers.GetLayerByCaption(Gis.getLayerLiquid());
            if( null != layer && null != liquid_layer){
                clearInterval(Index.showSensorMarkerItv);
